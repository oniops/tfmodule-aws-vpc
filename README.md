...
`context`는 [tfmodule-context](https://github.com/oniops/tfmodule-context) `v1.3.5` 이상의 출력을 그대로 받는다. 아래는 2 AZ, 워크로드 스택 1개, AZ별 NAT 1개의 최소 구성이다.
...
목적지가 여러 곳인 Flow Log 는 `flow_logs` 에 항목을 더한다. 항목 하나가 Flow Log 하나이며 빈 Map 이면 0개다. 로그 그룹·버킷·Delivery Stream 과 그 IAM 롤은 모듈이 만들지 않고 ARN 만 참조한다(RSC-FLOW-08).
...
`v3.0.0` 은 입력 `flow_log` 를 평면 Map `flow_logs` 로 바꾼다. 출력 이름과 키, 리소스 키와 이름 규칙은 그대로이므로 리소스가 재생성되지 않고 `moved` 블록도 필요 없다. 호출부에서 `destinations` 한 겹만 벗기면 된다.

```hcl
# v2
flow_log = {
  destinations = {
    s3 = { log_destination_type = "s3", log_destination_arn = "arn:aws:s3:::org-vpc-flowlogs/platform" }
  }
}
...
`vpc_endpoints` 는 기본값이 `null` 에서 `{}` 로 바뀌었으나 `nullable = false` 라 명시적 `null` 도 그대로 동작한다. 고칠 것이 없다.

각 입력의 전체 필드와 기본값은 `variables.tf`의 `description`에 호출 예시와 함께 있다. 스택 전용 Public, 보조 CIDR, NAT 인스턴스 ENI, NACL, VGW·CGW, IPv6, 3 AZ EKS 구성처럼 기능을 조합한 검증 입력 다섯 가지의 구성은 `requirements/REQUIREMENTS.md` 8.4절이 정의한다. EKS 스택에 필요한 서브넷 태그는 `requirements/ARCHITECTURE.md`에 있다.

## Input Variables

타입의 정본은 `requirements/ARCHITECTURE.md` 8.1·8.2절이다. 아래 표는 요약이며, 각 변수의 `description`에 호출 예시가 있다.

| 이름 | 타입 요약 | 기본값 | 설명 |
| --- | --- | --- | --- |
| `context` | `object` | 필수 | tfmodule-context `v1.3.5` 이상의 출력. `name_prefix`, `tags`, `region`, `pri_domain`만 쓰고 나머지 필드는 `optional()`로 받는다. `name_prefix`와 `tags`가 `null`이면 plan 실패 |
...
| `shared_public` | `object({ tags, subnets, nacl })` | `null` | Shared Public Network. 서브넷은 `igw` 기본 경로를 가진 RT를 가리켜야 한다 |
...
| `eni_interfaces` | `map(object)` | `{}` | 모듈이 만드는 ENI. NAT 인스턴스용은 `source_dest_check = false`. SG는 `security_group_names`(모듈 SG)와 `security_group_ids`(호출자 SG) 합집합이며 둘 다 기본 `[]` |
| `security_groups` | `map(object)` | `{}` | 모듈이 만드는 SG와 룰. 룰은 이름 키 Map이며 독립 리소스. 키 `vpce`는 예약 |
| `vpc_endpoints` | `object({ gateway, interface })` | `{}` | Gateway Endpoint는 모든 RT에 연결, Interface Endpoint는 `vpc_endpoint_subnets`에 ENI 생성. `nullable = false`라 명시적 `null`도 `{}`로 대체된다 |
| `vpn_gateway` | `object` | `null` | VGW 생성 또는 `existing_id` 연결. ASN은 문자열 |
| `customer_gateways` | `map(object)` | `{}` | Customer Gateway(`ipsec.1`). ASN은 문자열 |
| `flow_logs` | `map(object)` | `{}` | VPC Flow Log. 항목 하나가 Flow Log 하나(`cloud-watch-logs`·`s3`·`kinesis-data-firehose`)이고 빈 Map이면 0개다. 목적지 리소스는 만들지 않고 ARN만 참조한다 |
...
태그는 `merge(context.tags, tags, <인스턴스별 tags>, { Name })` 순서로 병합하고 `Name`은 보호 키다. plan에서 드러나지 않는 제약(미연결 ENI 경로, 룰 없는 SG, 기본 SG, `source_dest_check`)은 ARCHITECTURE 9.3절 표를 따른다.
...
| `flow_log_ids`, `flow_log_arns`, `flow_log_destination_arns` | `flow_logs` 키 → 값 |
...
REQUIREMENTS 8.4절 `eks` 기준 입력의 `toolchain` 스택 NACL 이 "Workload → Toolchain Default Deny" 를 NACL 로 구현한 예다. NACL 은 상태 비저장이고 룰의 포트 범위가 목적지 포트이므로, Toolchain 이 시작한 연결의 응답(목적지 포트 32768~65535)만 워크로드 대역에서 허용하고 나머지는 deny 한다. 32768 이상 포트로 연 서비스는 NACL 로 막을 수 없으므로 Security Group 으로 막는다.
...
절차와 판정 기준은 `requirements/REQUIREMENTS.md` 8절이 정의한다.
...
여기까지는 clone 직후 바로 돌아간다. 아래 두 단계는 검증 자산이 필요하다. 기준 입력(REQUIREMENTS 8.4절), `tests/*.tftest.hcl`(REQUIREMENTS 8.5절), `examples/<이름>/` 호출 스택은 `.gitignore`로 제외된 로컬 자산이라 저장소에 없으며, 요구사항 문서를 보고 로컬에서 만든 뒤 실행한다.
...
`apply`와 `destroy`는 이 저장소에서 실행하지 않는다. `terraform test`는 `mock_provider`로 실행해 AWS를 호출하지 않으므로 이 금지의 대상이 아니다. 검증 항목은 REQUIREMENTS 8.5절(`TST-01`~`TST-40`), 실패 케이스는 ARCHITECTURE 9.2절 두 표(`V-01`~`V-35`, `P-01`~`P-23`)를 따른다.
